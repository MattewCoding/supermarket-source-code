...
namespace HeneGames.Airplane
{
    ...
    public class SimpleAirPlaneController : MonoBehaviour
    {
        ...
        private bool inputRollLeft;
        private bool inputRollRight;
        ...
        private void Movement()
        {
            ...
            transform.Rotate(Vector3.up * inputH * currentYawSpeed * Time.deltaTime);
        ...
        {
            // Use the Right analog stick for yaw (horizontal axis) and roll (vertical axis)
            inputH = OVRInput.Get(OVRInput.Axis2D.PrimaryThumbstick).x;
            inputV = OVRInput.Get(OVRInput.Axis2D.PrimaryThumbstick).y;

            // Use the B and A buttons for pitch control
            inputRollLeft = OVRInput.Get(OVRInput.Button.Two);  // B button (leftward yaw)
            inputRollRight = OVRInput.Get(OVRInput.Button.One); // A button (rightward yaw)

            // Turbo
            inputTurbo = OVRInput.Get(OVRInput.Button.PrimaryThumbstick); // Optional: You can assign a specific button to trigger turbo if needed
